...
check during the build; its explicit manual mode uses only a disposable local
platform credential and never opens a vault. Tests use uniquely identifiable
disposable values; do not use the current build with real credentials.
